...
macho_segment_t* macho_segment_create() {
	macho_segment_t* segment = malloc(sizeof(macho_segment_t));
	if (segment) {
...
macho_segment_t* macho_segment_load(unsigned char* data, unsigned int offset) {
	...
	if (segment) {
		segment->cmd = macho_segment_cmd_load(data, offset);
		if (!segment->cmd) {
			macho_segment_free(segment);
			return NULL;
		}
		segment->name = strdup(segment->cmd->segname);
		segment->size = segment->cmd->cmdsize;
		segment->offset = segment->cmd->fileoff;
		segment->address = segment->cmd->vmaddr;
		segment->data = NULL; // ????
